...
module.exports = {
	...
	async getEventByUserIDAndTime(req, res) {
		try {
			...
			const events = await eventModel.getEventByUserIDAndTime(
				user_id,
				gte,
				lte
			);
	...
	async createEvent(req, res) {
		try {
			if (req.body.title && req.body.content && req.body.date) {
				...
				const createEvent = await eventModel.createEvent(
					title,
					content,
					dateFormat,
					id_author_event
	...
	async updatEventByID(req, res) {
		try {
			const eventAutorId = await eventModel.getEventByID(
				req.params.event_id
			);
			const event_id = req.params.event_id;
			const { title, content, date } = req.body;
			...
			if (req.user.id === eventAutorId[0].id) {
				if (title) {
					const updateTitleByID = await eventModel.updateTitleByID(
						event_id,
						title
					);
				}
				if (content) {
					const updateDescriptionByID =
						await eventModel.updateDescriptionByID(
							event_id,
							content
						);
				}
				if (date) {
					const dateFormat = moment(date, 'DD MM YYYY').format('X');
					const updateDateByID = await eventModel.updateDateByID(
						event_id,
						dateFormat
					);
				}
